  ...
  return (
    <div className="w-full min-h-screen bg-background">
      ...
      <div className="flex flex-col md:flex-row">
        <div className="flex flex-col pl-4 pr-4 md:pl-10 md:pr-10">
                  ...
                  <table className="w-full bg-white shadow-lg">
                    <thead>
                      <tr className="bg-gray-200">
                        <th className="py-2 md:py-3 px-3 md:px-5">Product</th>
                        <th className="py-2 md:py-3 px-3 md:px-5">Name</th>
                        <th className="py-2 md:py-3 px-3 md:px-5">Quantity</th>
                        <th className="py-2 md:py-3 px-3 md:px-5">Price</th>
                        <th className="py-2 md:py-3 px-3 md:px-5">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {cart.items.map((item, index) => (
                        <tr key={index} className="border-b">
                          <td className="py-2 md:py-3 px-3 md:px-5">
                            {item.image ? (
                              <Image
                                src={item.image}
                                alt={item.name}
                              ...
                              <div className="w-16 h-16 md:w-20 md:h-20 flex items-center justify-center bg-gray-200">
                                No Image Available
                              </div>
                            )}
                          </td>
                          <td className="py-2 md:py-3 px-3 md:px-5">{item.name}</td>
                          <td className="py-2 md:py-3 px-3 md:px-5 flex justify-center items-center">
                            ...
                            <input
                              ...
                              className="w-12 md:w-16 px-2 md:px-3 text-center mx-2"
                            />
                            <button className="plus-btn" onClick={() => handlePlusClick(index)}>
                              +
                            </button>
                          </td>
                          <td className="py-2 md:py-3 px-3 md:px-5">{item.price * item.count} Rs</td>
                          <td className="py-2 md:py-3 px-3 md:px-5">
                            <AiOutlineClose
                              className="cursor-pointer p-1 h-6 w-6 md:h-7 md:w-7"
        ...
        {cart.items.length > 0 && (
          <div className="flex justify-end ml-auto mr-5 mt-10 md:mt-32 bg-background border-[0.5px] border-secondary_color p-5 md:p-10 w-full md:w-[20rem]">
            <div className="w-full flex flex-col justify-center items-center text-lg font-normal">
              ...
              <div className="w-full items-center justify-center flex pt-5">
                <Link href="/checkout">
                  <button className="px-4 py-2 md:px-5 md:w-40 md:py-2 border-2 border-black dark:border-white uppercase bg-secondary_color text-white transition duration-200 text-sm shadow-lg dark:shadow-white">
                    CheckOut →
                  </button>
                </Link>
              </div>
